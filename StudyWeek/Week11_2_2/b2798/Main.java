/*
🖥️[브루트포스] 2798번 문제 : 블랙잭
🖥️https://www.acmicpc.net/problem/2798
...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < card; i++) {
            dum.add(Integer.parseInt(st.nextToken()));
        }

        dum.sort(Comparator.naturalOrder());
        ...
        for (int i = 0; i < card; i++) {
            for (int j = i+1; j < card; j++) {
                for (int k = j+1; k < card; k++) {
                    sum = dum.get(i) + dum.get(j) + dum.get(k);
                    if(sum <= target) {
                        sums.add(sum);
                    }
                }
